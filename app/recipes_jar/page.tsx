...
export default function CookPotJar() {
  ...
  const sortedRecipes = useMemo(() => {
    ...
    arr.sort((a: any, b: any) => {
      ...
      switch (sortType) {
        case "alphabet":
          valA = t(`recipes_jar.${a.name}`) ?? "";
          valB = t(`recipes_jar.${b.name}`) ?? "";
              ...
              >
                <div className="max-h-80 overflow-y-auto overscroll-contain hide-scrollbar">
                  ...
                  {searchedRecipes.map((recipe, idx) => (
                    ...
                    >
                      <img
                        src={getAssetPath(`/foods_cookpot_jar/${recipe.name}.png`)}
          ...
          >
            ...
            {(filtersOpen || sortingOpen) && (
              <div className="absolute top-full mt-3 left-1/2 -translate-x-1/2 flex items-start gap-4 z-50">
                {/* FILTER PANEL */}
                {filtersOpen && (
                  <div className="w-11/12 sm:w-[420px] bg-white dark:bg-zinc-900 border border-zinc-300 dark:border-zinc-700 rounded-xl p-4 font-bold shadow-sm dark:shadow-none">
            ...
            <div className="flex items-center justify-start w-full">
              {recipe.pigcoinvalue.map((value, index) =>
                value > 0 ? (
                  <img
                    key={index}
                    src={getAssetPath(`/icons/cooking/icon_coin${index + 1}.png`)}
                    className="w-7 h-7"
                    alt=""
                    />
                  ) : null
      ...
      <AnimatedOverlay isOpen={!!selected} onClose={() => setSelected(null)}>
        {selected && (<>
          ...
          >
            ...
            <SkeletonImage
              src={getAssetPath(`/foods_cookpot_jar/${selected.name}.png`)}
              className="w-24 h-24 mx-auto mb-4"
            />

            <h2 className="text-center text-2xl font-semibold">
              {t(`recipes_jar.${selected.name}`)}
            </h2>
            ...
            {(() => {
              ...
              return suggestion ? (
                <SeeAlso
                  ...
                  imageBasePath="/foods_cookpot_jar"
                  translationPrefix="recipes_jar"
                  onSelect={(recipe) => {
                    setSelected(null);
                    selectRecipe(recipe);
                  }}
